...
import { logBalanceCheck } from '../services/balanceService'

const SIMULATED_SCORE = 74
  ...
  useEffect(() => {
    if (phase === 'measuring') {
      if (countdown <= 0) {
        setPhase('analyzing')
        setTimeout(() => {
          logBalanceCheck(userId, { swayScore: SIMULATED_SCORE })
            .catch(() => setError('Score could not be saved.'))
          setPhase('results')
        }, 700)
        return
      }
  ...
  return (
    <div className="flex-1 flex flex-col px-5 pt-6 pb-8">
      <div className="flex-1 flex flex-col">
        {error && <p className="text-sm text-accent-red mb-3 text-center">{error}</p>}
        ...
        <div className="bg-white border border-surface-border rounded-card p-5 mb-4 text-center">
          ...
          <div className="flex items-end justify-center gap-1 mb-2">
            <span className="font-serif text-[52px] leading-none font-semibold text-teal">{SIMULATED_SCORE}</span>
        ...
        <div className="bg-white border border-surface-border rounded-card p-5 mb-4">
          ...
          <div className="relative h-4 rounded-full overflow-hidden mb-2" style={{
            ...
            <div
              className="absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-white border-2 border-text-primary shadow-sm"
              style={{ left: `calc(${SIMULATED_SCORE}% - 8px)` }}
            />
          </div>
        ...
        <button onClick={() => {}} className="text-sm text-text-muted text-center hover:text-text-secondary mb-4">
